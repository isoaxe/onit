...
export async function create(
  ...
  try {
    const role = "staff";
    const { displayName, phoneNumber, email, password, lastName, businessId } =
      req.body;
  ...
  try {
    ...
    if (numAuthUsers !== numFirestoreUsers) {
      return res.status(500).send({
        error: `There are ${numAuthUsers} users in the Auth data and ${numFirestoreUsers} in Firestore. This mismatch was likely caused by the admin deleting user data from one source in the Firebase console without deleting the corresponding data from the other source.`,
      });
    }
...
function mapUser(user: admin.auth.UserRecord) {
  const customClaims = (user.customClaims || { role: "", businessId: "" }) as {
    role?: string;
    businessId?: string;
  };
